...
export type UserType = {
  ...
  name: string;
  age: number;
  address: AddressType//any; // пропиши типизацию
};

export type UsersObjectType = {
  myFriends: Array<UserType>//any; // пропиши типизацию
};
...
export const HW2 = () => {
  ...
  const filterUsers = () => {
    const filteredUsers = users.myFriends.filter((a)=> a.address.city === 'Los Angeles' ) //'НУЖНО ПРОФИЛЬТРОВАТЬ ДРУЗЕЙ. ОСТАВЛЯЕМ ТОЛЬКО ТЕХ, КОТОРЫЕ ЖИВУТ В ГОРОДЕ LOS ANGELES';
